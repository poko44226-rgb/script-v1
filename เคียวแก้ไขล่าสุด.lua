...
local function GetBallAsync()
    task.spawn(function()
        ...
        part.CanCollide = true -- เปิดให้ชนตัวละคร
        part.Massless = true
...
-- ฟังก์ชัน Fling แก้ไขจุดลอยบนหัว + ใส่แรงสั่นสะเบ็ด
local function flingTarget(ball, targetPos, originPos)
    if not (ball and ball.Parent) then return end

    ball.Anchored = false
    ball.CanCollide = true -- เปิดการชนเพื่อให้แรงกระแทกทำงานได้
    ...
    -- วางตำแหน่งบอลกระแทกใส่จุดเป้าหมายโดยตรง
    local jitter = Vector3.new(math.random(-2, 2), math.random(-2, 2), math.random(-2, 2))
    ball.CFrame = CFrame.new(targetPos + jitter)

    -- ส่งแรงสั่นกระแทกมหาศาล (Velocity)
    local launchVelocity = direction * 35000 + Vector3.new(0, 20000, 0) + Vector3.new(

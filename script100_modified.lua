...
-- ==================== ระบบสลับสี ====================
local currentTheme = "RED" -- ค่าเริ่มต้นเป็นสีแดง
...
local THEMES = {
    ...
    BLUE = {
        Abyss = Color3.fromRGB(0, 0, 80),
        Toxic = Color3.fromRGB(0, 100, 255),
        Deep  = Color3.fromRGB(0, 0, 40),
        Name  = "SWITCH RED"
    }
}

-- ตัวช่วยดึงสีปัจจุบัน
...
-- ใช้ฟังก์ชันสร้าง Gradient ใหม่เพื่อให้อัปเดตสีได้
...
-- ==================== ปุ่มสลับสี ====================
local bColorToggle = createBtn(THEMES.RED.Name, UDim2.new(0.05, 0, 0, 185))
bColorToggle.TextSize = 9

-- ฟังก์ชันอัปเดตสีทั้ง GUI เมื่อสลับ Theme
local function applyTheme()
    -- อัปเดต Gradient หลัก
    updateMainGradient()
    ...
    -- อัปเดตข้อความปุ่ม
    bColorToggle.Text = THEMES[currentTheme].Name

    -- อัปเดตสีปุ่ม Active ตามสถานะปัจจุบัน
    if isActivated then

...
local function GetBall()
    ...
    if not b then 
        ...
        task.wait(0.5)
        b = LP.Backpack:FindFirstChild("SoccerBall") or (LP.Character and LP.Character:FindFirstChild("SoccerBall"))
    end
    
    if not b then return nil end
    
    if b:IsA("Tool") and b.Parent == LP.Backpack then
        LP.Character:FindFirstChildOfClass("Humanoid"):EquipTool(b)
        task.wait(0.3)
    end
    
    local ball = workspace:WaitForChild("WorkspaceCom"):WaitForChild("001_SoccerBalls"):WaitForChild("Soccer" .. LP.Name, 5)
    if ball then
...
local function flingTarget(ball, targetPos, originPos)
    ...
    ball.CFrame = CFrame.new(targetPos + Vector3.new(0, 3, 0))
    
    local velocity = direction * 15000 + Vector3.new(0, 10000, 0)
    ...
    ball.RotVelocity = Vector3.new(math.random(-500, 500), math.random(-500, 500), math.random(-500, 500))
end
...
Main.Size, Main.Position = UDim2.new(0, 160, 0, 280), UDim2.new(0.5, -80, 0.4, 0)
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    local targetSize = isMinimized and UDim2.new(0, 160, 0, 35) or UDim2.new(0, 160, 0, 280)
...
RS.Heartbeat:Connect(function()
    ...
    if not root or not petMode or isAttacking or skill2Active or soulSlashActive or deathSpinActive or #wingRemoteCache < 1 then return end

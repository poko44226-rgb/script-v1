...
-- UI หลัก: ขนาดกะทัดรัด 230x275
local Main = Instance.new("Frame", Gui)
Main.Size = UDim2.new(0, 230, 0, 275)
...
local bActivate = createBtn("ACTIVATE CONTROL", UDim2.new(0.05, 0, 0, 45))
local bSummon = createBtn("SUMMON ABYSS WINGS", UDim2.new(0.05, 0, 0, 82))
local bDismantle = createBtn("GET DISMANTLE", UDim2.new(0.05, 0, 0, 119))
local bColorToggle = createBtn("COLOR: RED", UDim2.new(0.05, 0, 0, 156))
...
-- ==================== COLOR PICKER: STANDALONE UI ====================
-- Color Picker เป็น ScreenGui แยกจาก Main โดยสมบูรณ์
local ColorPickerGui = Instance.new("ScreenGui")
ColorPickerGui.Name = "TrueShadow_ColorPicker"
ColorPickerGui.DisplayOrder = Gui.DisplayOrder + 1
ColorPickerGui.IgnoreGuiInset = true
ColorPickerGui.ScreenInsets = Enum.ScreenInsets.None
...
closePicker.AutoButtonColor = true
...
local function setColorPickerVisible(visible)
    ...
    if visible then
        -- Picker เป็น UI คนละตัว: ไม่ซ่อน/ไม่ resize Main และไม่แก้ Content
        DropdownList.Visible = false
        ...
        if x + pickerW > viewport.X - 8 then
            x = mainPos.X - pickerW - gap
        end
        if x < 8 then x = 8 end
        if y + pickerH > viewport.Y - 8 then
            y = math.max(8, viewport.Y - pickerH - 8)
        end
...
for index, theme in ipairs(COLOR_THEMES) do
    ...
    colorBtn.Size = UDim2.fromOffset(45, 36)
    ...
    colorBtn.Activated:Connect(function()
        ...
        applyTheme()
        -- ไม่ปิด Color Picker หลังเลือกสี
        -- ผู้ใช้สามารถเลือกสีอื่นต่อได้ทันที
    end)
end
...
bColorToggle.Activated:Connect(function()
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    TS:Create(Main, TweenInfo.new(0.4, Enum.EasingStyle.Quart), {
        Size = isMinimized and UDim2.new(0, 230, 0, 40) or UDim2.new(0, 230, 0, 275)
    }):Play()
    Content.Visible = not isMinimized
    if isMinimized then
        DropdownList.Visible = false
        -- Color Picker เป็น UI แยก จึงไม่ปิดเมื่อพับ UI หลัก
    end
end)

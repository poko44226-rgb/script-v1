...
-- [[ MINECRAFT WITHER BOSS - SIT FLING EDITION V7.3.2 - BLOCK RELEASE FIX ]] --
...
titleLabel.BackgroundTransparency, titleLabel.Text = 1, "Wither Sit-Fling V7.3.2 | RELEASE FIX"
                ...
                if projData.IsStuck then
                    if sitFlingMode then
                        if not projData.VoidDragStarted then
                            ...
                            -- [[ FIX V7.3.2: เป้าหลุดจากเหวแล้ว (ลุกจากเก้าอี้/reset/กลับเกาะ) ให้ปล่อยบล็อกกลับ ]]
                            local vHrp = vChar and vChar:FindFirstChild("HumanoidRootPart")
                            local victimStillInVoid = vHrp and (vHrp.Position.Y < SOAK_RELEASE_Y)

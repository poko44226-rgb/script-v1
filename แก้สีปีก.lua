...
local COLORS = {
    Abyss = Color3.fromRGB(80, 0, 0),
    Toxic = Color3.fromRGB(255, 0, 0),
    Deep  = Color3.fromRGB(40, 0, 0),
    BlueAbyss = Color3.fromRGB(0, 0, 80),
    BlueToxic = Color3.fromRGB(0, 0, 255),
    BlueDeep  = Color3.fromRGB(0, 0, 40)
}

local isRedColorMode = true

local function getAbyssColor(t, offset)
    local cycle = (t * 0.3 + (offset * 0.1)) % 1
    if isRedColorMode then
        if cycle < 0.5 then
        ...
        if cycle < 0.5 then
            return COLORS.BlueAbyss:Lerp(COLORS.BlueToxic, cycle * 2)
        else
            return COLORS.BlueToxic:Lerp(COLORS.BlueDeep, (cycle - 0.5) * 2)
        end
    end
end
...
MainGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, COLORS.Abyss),
    ColorSequenceKeypoint.new(0.5, COLORS.Toxic),
    ColorSequenceKeypoint.new(1, COLORS.Deep)
})
...
Title.TextStrokeColor3 = COLORS.Deep
...
DropdownList.BackgroundColor3 = COLORS.Deep
...
local colorToggleBtn = Instance.new("TextButton", Content)
colorToggleBtn.Size = UDim2.new(0.9, 0, 0, 30)
colorToggleBtn.Position = UDim2.new(0.05, 0, 0, 210)
colorToggleBtn.Text = "COLOR: RED"
colorToggleBtn.TextColor3 = Color3.new(1, 1, 1)
colorToggleBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
colorToggleBtn.TextSize = 14
colorToggleBtn.Font = Enum.Font.GothamBold
Instance.new("UICorner", colorToggleBtn)

colorToggleBtn.MouseButton1Click:Connect(function()
    isRedColorMode = not isRedColorMode

    if isRedColorMode then
        colorToggleBtn.Text = "COLOR: RED"
        colorToggleBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
    else
        colorToggleBtn.Text = "COLOR: BLUE"
        colorToggleBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 100)
    end
end)
...
bActivate.MouseButton1Click:Connect(function()
    ...
    if isActivated then
        bActivate.BackgroundColor3 = COLORS.Abyss
...
bSummon.MouseButton1Click:Connect(function()
    ...
    if wingsActive then
        ...
        bSummon.BackgroundColor3 = COLORS.Abyss
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    TS:Create(Main, TweenInfo.new(0.4, Enum.EasingStyle.Quart), {
        Size = isMinimized and UDim2.new(0, 200, 0, 40) or UDim2.new(0, 200, 0, 230)
    }):Play()
    Content.Visible = not isMinimized
    if isMinimized then DropdownList.Visible = false end
end)
...
-- [[ MINECRAFT WITHER BOSS - SIT FLING EDITION V7.4.0 ]] --
-- Engine Core: DYTON_txt (Spawn-Behind + Sweep Void Drag Fix)
-- Feature: Spawn behind player, sweep-drag sitting targets to void
...
local ATTACK_COOLDOWN = 1.3
local HOVER_HEIGHT = 24
local RANDOM_DRIFT_SCALE = 50
local RAMPAGE_RADIUS = 400
local SUMMON_BEHIND_DIST = 18 -- v7.4: ระยะเกิดวิเทอร์ 'ข้างหลัง' ตัวละคร (studs) ปรับได้
...
-- วิเทอร์ลอยตามด้านหลังผู้ใช้ (แกน +Z = ด้านหลังตัวละคร) เว้นระยะห่าง กันบอลจอดใกล้ตัวแล้วเผลอไปนั่ง
local PET_OFFSET = Vector3.new(0, 7, 10)
...
local DRAG_VELOCITY = Vector3.new(0, -9e10, 0)
local UNSEAT_FORCE = Vector3.new(0, 5000, 0)
local SEAT_PARK_OFFSET = Vector3.new(0, 1.5, 0) -- ตำแหน่งจอดบอลรอ (ใต้ลำตัวเป้าเล็กน้อย)
local VOID_RETURN_DELAY = 2.0 -- เวลาค้างใต้แมพ (-1000) ก่อนยกกลับขึ้นมา (วินาที)
-- ===================================

-- [[ SMART PAUSE CONFIG ]] --
local FALLEN_THRESHOLD = -1000
local sitFlingPaused = false
local SIT_FLING_DELAY = 0.2 -- ดีเลย์หลังเป้านั่งนิ่ง ก่อนดึงลง void (วินาที)
local targetSitStartTime = 0
local voidDragStartTime = 0
local voidDragActive = false -- v7.4: true = กำลังอยู่ในช่วงดึงเป้าลง void
-- ===========================
...
titleLabel.BackgroundTransparency, titleLabel.Text = 1, "Wither Sit-Fling V7.4.0"
    ...
    if enterPressed then
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p.Name:lower():find(searchKey) or p.DisplayName:lower():find(searchKey) then
                ...
                targetSitStartTime = 0
                voidDragActive = false
                voidDragStartTime = 0
...
SummonBtn.MouseButton1Click:Connect(function()
    ...
    if petMode then
        ...
        if hrp then
            -- v7.4: เกิด 'ข้างหลัง' ตัวละคร (+Z ของ character space) แทนการเกิดใต้เท้า
            summonOriginPos = (hrp.CFrame * CFrame.new(0, 0, SUMMON_BEHIND_DIST)).Position
...
SitFlingBtn.MouseButton1Click:Connect(function()
    ...
    if sitFlingMode then
        ...
        sitStatusLabel.Text = "SIT FLING: ON | DRAG TO VOID"
...
RS.Heartbeat:Connect(function(dt)
    ...
    -- [[ SIT DELAY TRACKER - Wait for sit to fully settle before void dragging ]] --
    if TargetPlayer then
        local tChar = TargetPlayer.Character
        local tHum = tChar and tChar:FindFirstChildOfClass("Humanoid")
        if tHum and tHum.Sit then
            if targetSitStartTime == 0 then
                targetSitStartTime = t
            end
        else
            targetSitStartTime = 0
        end
    ...
    -- [[ SMART PAUSE SYSTEM V7.2 - Pause when seated target falls below map ]] --
    if sitFlingMode and TargetPlayer then
        local tChar = TargetPlayer.Character
        local tHrp = tChar and tChar:FindFirstChild("HumanoidRootPart")
        local tHum = tChar and tChar:FindFirstChildOfClass("Humanoid")
        local isUnderMap = tHrp and tHrp.Position.Y < FALLEN_THRESHOLD
        local isSitting = tHum and tHum.Sit

        if isUnderMap and isSitting and targetSitStartTime > 0 and (t - targetSitStartTime) >= SIT_FLING_DELAY then
            sitFlingPaused = true
        end

        if sitFlingPaused then
            -- Resume when target comes back to island OR gets up from seat
            if not isUnderMap or not isSitting then
                sitFlingPaused = false
                targetSitStartTime = 0
                voidDragActive = false
                voidDragStartTime = 0
    ...
    -- [[ ANTI SELF-SIT v7.3: กันตัวเองเผลอไปนั่งบนบอลตอนบอลวาปมาใกล้ ]] --
    local ballRef = GetBallSilent()
    if sitFlingMode and ballRef then
        local myChar = LP.Character
        local myHum = myChar and myChar:FindFirstChildOfClass("Humanoid")
        local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
        if myHum and myHum.Sit and myRoot and (ballRef.Position - myRoot.Position).Magnitude < 12 then
            pcall(function() myHum.Sit = false end)
            myRoot.AssemblyLinearVelocity = Vector3.new(myRoot.AssemblyLinearVelocity.X, 40, myRoot.AssemblyLinearVelocity.Z)
        end
    end

    -- [[ VOID RETURN / DRAG HOLD SYSTEM v7.4 ]] --
    -- บอลจมใต้แมพ: ถ้ากำลังดึงเป้าอยู่ (voidDragActive) ให้บอลเกาะเป้าต่อเนื่อง
    -- ถ้าไม่มีดรากคุม แปลว่าบอลหลงไปใต้แมพ ยกกลับมาหาวิเทอร์ทันที (แบบเดิม)
    if ballRef and ballRef.Position.Y < FALLEN_THRESHOLD then
        local victimHrp = nil
        if voidDragActive and TargetPlayer then
            pcall(function()
                local tCharV = TargetPlayer.Character
                victimHrp = tCharV and tCharV:FindFirstChild("HumanoidRootPart")
            end)
        end

        if voidDragActive and victimHrp then
            -- เกาะเป้าไว้ใต้แมพ พร้อมความเร็วลงต่อเนื่อง จนกว่าจะครบเวลา
            ballRef.CanCollide = true
            ballRef.CFrame = CFrame.new(victimHrp.Position)
            ballRef.Velocity = DRAG_VELOCITY
            ballRef.AssemblyLinearVelocity = DRAG_VELOCITY

            -- ครบเวลาค้างใต้แมพ ยกบอลกลับขึ้นมาใต้วิเทอร์
            if (t - voidDragStartTime) >= VOID_RETURN_DELAY then
                local returnPos = witherCurrentPos + Vector3.new(0, -HOVER_HEIGHT + 3, 0)
                ballRef.CanCollide = false
                ballRef.CFrame = CFrame.new(returnPos)
                ballRef.Velocity = Vector3.zero
                ballRef.AssemblyLinearVelocity = Vector3.zero
                table.clear(activeProjectiles)
                voidDragActive = false
                targetSitStartTime = 0
                voidDragStartTime = 0
            ...
            -- ไม่มีดรากคุม หรือเป้าหายตัว (ตาย/รีเซ็ต): ยกบอลกลับมาหาวิเทอร์
            voidDragActive = false
            voidDragStartTime = 0
            ballRef.CanCollide = false
            local returnPos = witherCurrentPos + Vector3.new(0, -HOVER_HEIGHT + 3, 0)
    ...
    elseif not voidDragActive then
        voidDragStartTime = 0
    end

    if sitFlingPaused then
        ballStatusLabel.Text = "WITHER: PAUSED (TARGET SEATED/VOID)"
        ballStatusLabel.TextColor3 = COLORS.SitFlingPurple
        sitStatusLabel.Text = "SIT FLING: PAUSED"
        sitStatusLabel.TextColor3 = COLORS.SitFlingPurple

        -- Keep wings visible but frozen in place + still breathe colors
        ...
        return -- Skip all wither logic until resumed
    end
    -- ===========================================================================
    ...
    elseif flingActiveThisFrame then
        if sitFlingMode then
            ballStatusLabel.Text = "BALL: VOID DRAGGING TARGET "
            ballStatusLabel.TextColor3 = COLORS.SitFlingPurple
        else
    ...
    if not isSummoning then
        ...
        elseif targetTorso then
            ...
            if isPetState then
                ...
                idealTargetPos = targetTorso.Position + Vector3.new(driftX, HOVER_HEIGHT + driftY, driftZ)
            end
            ...
            if isPetState then
                ...
                rawLookAt = Vector3.new(targetTorso.Position.X, witherCurrentPos.Y, targetTorso.Position.Z)
            end
    ...
    if not isSummoning then
        for auraId, projData in pairs(activeProjectiles) do
            local currentEnemyTorso = getTargetTorso(TargetPlayer)

            if currentEnemyTorso then
                local enemyVelocity = currentEnemyTorso.AssemblyLinearVelocity
                local pingAndFpsOffset = dt + 0.05
                local predictedPosition = currentEnemyTorso.Position + (enemyVelocity * pingAndFpsOffset)

                if projData.IsStuck then
                    -- [[ SIT FLING / VOID DRAG LOGIC (v7.4 - Sweep Drag) ]] --
                    local sitReady = sitFlingMode and targetSitStartTime > 0 and (t - targetSitStartTime) >= SIT_FLING_DELAY
                    local dragging = sitReady or (voidDragActive and voidDragStartTime > 0)

                    if dragging then
                        -- v7.4 SWEEP: เกาะบอลกับตัวเป้าทุกเฟรม + ความเร็วลงสูงสุดต่อเนื่อง
                        -- แรงปะทะสะสมจะพุ่งเป้าลงหลุม แทนการวาปแบบเดิมที่เป้าไม่มีทางตาม
                        local victimPos = currentEnemyTorso.Position
                        projData.CurrentPos = victimPos
                        voidDragActive = true
                        if voidDragStartTime == 0 then voidDragStartTime = t end

                        if activeBall then
                            activeBall.CanCollide = true
                            activeBall.CFrame = CFrame.new(victimPos)
                            activeBall.Velocity = DRAG_VELOCITY
                            activeBall.AssemblyLinearVelocity = DRAG_VELOCITY
                        end

                        -- ลองดึงให้เป้าลุกจากเก้าอี้ (ได้ผลเฉพาะถ้า character เป้าอยู่ในเน็ตเวิร์คเรา)
                        pcall(function()
                            local enemyChar = TargetPlayer.Character
                            local enemyHum = enemyChar and enemyChar:FindFirstChildOfClass("Humanoid")
                            if enemyHum and enemyHum.Sit then
                                enemyHum.Sit = false
                            end
                        end)
                        -- ห้ามหมดอายุระหว่างดราก ให้ระบบ VOID RETURN เป็นคนจบดรากเอง
                    elseif sitFlingMode then
                        -- [[ WAITING FOR SIT: park ball under target, ZERO velocity, no warp ]] --
                        projData.CurrentPos = predictedPosition + SEAT_PARK_OFFSET
                        if activeBall then
                            activeBall.CanCollide = false
                        ...
                        -- Reset expiry so the ball keeps holding until target actually sits
                        projData.ImpactTime = t
                    else
                        -- Normal fling (sitFling off) - unchanged behavior
                    ...
                    if pDist > 1.8 then
                        ...
                        local sitReadyAtImpact = sitFlingMode and targetSitStartTime > 0 and (t - targetSitStartTime) >= SIT_FLING_DELAY

                        if sitReadyAtImpact then
                            -- v7.4: เริ่มดรากทันทีแบบ SWEEP (เกาะเป้า + กระแทกลงต่อเนื่อง)
                            -- การวาปไป -1000 แบบเดิมไม่มีแรงปะทะต่อเนื่อง เป้าจะไม่ตามลงไป
                            voidDragActive = true
                            if voidDragStartTime == 0 then voidDragStartTime = t end
                            projData.CurrentPos = predictedPosition
                            if activeBall then
                        ...
                        elseif sitFlingMode then
                            -- Sit fling ON but target not seated yet: PARK the ball, do NOT warp
                            projData.CurrentPos = predictedPosition + SEAT_PARK_OFFSET
                            if activeBall then
                                activeBall.CFrame = CFrame.new(projData.CurrentPos)
                                activeBall.Velocity = Vector3.zero
                            ...
                            -- Normal fling on impact (sitFling off)
                            projData.CurrentPos = predictedPosition
                            if activeBall then
                                activeBall.CFrame = CFrame.new(predictedPosition)
                                activeBall.Velocity = Vector3.new(0, 9e10, 0)
    ...
    if TargetPlayer and getTargetTorso(TargetPlayer) and not isSummoning then
        if t - lastAttackTime > ATTACK_COOLDOWN then
            ...
            if freeCount > 0 then
                local burstCount = math.random(1, math.min(3, freeCount))
                local enemyTorso = getTargetTorso(TargetPlayer)

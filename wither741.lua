...
-- [[ MINECRAFT WITHER BOSS - SIT FLING EDITION V7.4.0 - SIT-SNATCH ]] --
-- Fix: Reset TargetPlayer & all states when toggling petMode ON/OFF
...
local SOAK_RELEASE_Y = -800 -- เป้าสูงกว่าระดับนี้ = หลุดจากเหวแล้ว ปล่อยบล็อกกลับ
local SOAK_GRACE_TIME = 1.5 -- ช่วง grace: ลากเต็มที่ก่อน ค่อยเริ่มเช็คว่าเป้าหลุดหรือยัง
local SIT_SNATCH_RADIUS = 9 -- ใครนั่งอยู่ในรัศมีนี้จากบล็อก = ถือว่านั่งทับบล็อก
-- ===================================

-- [[ NPC CHASE CONFIG - ABSOLUTE LOCK-ON TRACKING ]] --
...
titleLabel.BackgroundTransparency, titleLabel.Text = 1, "Wither Sit-Fling V7.4.0 | SNATCH"
...
creditLabel.BackgroundTransparency, creditLabel.Text = 1, "By DYTON_txt | State Fix"
...
SummonBtn.MouseButton1Click:Connect(function()
    petMode = not petMode
    if petMode then
        -- [[ FIX: RESET ALL STATES WHEN TURNING ON ]] --
        TargetPlayer = nil
        rampageMode = false
        sitFlingMode = false
        sitFlingPaused = false
        ...
        -- [[ FIX: RESET ALL STATES WHEN TURNING OFF ]] --
        TargetPlayer = nil
        rampageMode = false
        sitFlingMode = false
        sitFlingPaused = false
...
-- [[ V7.4 SIT-SNATCH: หาคนที่นั่งทับบล็อก/ลูกจริง ๆ ]] --
        ...
        for auraId, projData in pairs(activeProjectiles) do
            ...
            if not vTorso or not vHum or vHum.Health <= 0 or not Players:FindFirstChild(victim.Name) then
                ...
                if projData.IsStuck then
                    if sitFlingMode then
                        -- [[ V7.4: ใครนั่งทับบล็อกจริง สลับเป้าเป็นคนนั้นเลย ]]
                        local sitter = FindSitterNear(projData.CurrentPos)
                        if sitter and sitter ~= projData.Victim then
                            ...
                            if not projData.VoidDragStarted then
                                projData.ImpactTime = t -- รีเวลา hold ให้คนใหม่เต็ม ๆ
                            end
                        end
                        ...
                        if not projData.VoidDragStarted then
                            ...
                            -- [[ FIX V7.3.3: ให้ช่วง grace ลากเต็มที่ก่อน ค่อยเช็คว่าเป้าหลุดจากเหว ]]
        ...
        if canFire and t - lastAttackTime > ATTACK_COOLDOWN then
            ...
            if freeCount > 0 then
                if rampageMode then
                    ...
                    -- [[ V7.3.4: โหมด Sit Fling ยิงทีละ 1 บล็อกต่อเป้า 1 คน ]]
                    local burstCount = sitFlingMode and 1 or math.random(1, math.min(2, freeCount))
                    local enemyTorso = getTargetTorso(TargetPlayer)

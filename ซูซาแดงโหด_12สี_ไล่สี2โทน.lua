--[[
    ...
    - [UPDATE] ทุกสีไล่สลับ 2 โทนอัตโนมัติ (เหมือนโหมดสีแดง) เอาปุ่ม AUTO ออก
]]
...
-- 12 แม่สี + โหมดออโต้
-- แต่ละสีมี 2 โทน (c1 = โทนปกติ, c2 = โทนเข้ม) สำหรับไล่สีสลับเหมือนสีแดง
local presetColors = {
...
-- คู่สีที่เลือกไว้ (nil = โหมดสุ่มออโต้เดิม) | เก็บ 2 โทนเพื่อไล่สี
...
for idx, data in ipairs(presetColors) do
    ...
    cb.MouseButton1Click:Connect(function()
        ...
        -- เริ่มไล่สีทันที: สุ่มโทนแรกให้ทุก prop
        for i = 1, 15 do
...
RS:BindToRenderStep("CURSED_HAND_ENGINE", 1, function(dt)
    ...
    for i = 1, math.min(#wingRemoteCache, 15) do
        ...
        if prop and prop.Parent and rawCF then
            ...
            -- [อัปเดต] สี: ถ้าเลือกสีไว้จะไล่สลับ 2 โทน ถ้าไม่เลือกจะสุ่ม 3 โทนซูซาโนะเดิม (ความไวเท่ากัน)

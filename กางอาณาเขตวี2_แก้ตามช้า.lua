    ...
    for _, cat in pairs(wsCom:GetChildren()) do
        for _, p in pairs(cat:GetChildren()) do
            if p:FindFirstChild("SetCurrentCFrame") then 
                ...
                controlledProps[p] = true -- จำ prop ที่เป็นกับดักของเรา
            end
        end
    end
    remoteCache = temp
...
-- [FIX LAG] อัปเดต prop ทีละ BATCH ตัวต่อเฟรม แทนทีละ 1 ตัว
-- ค่าเดิมอัปเดตทีละ 1 ต่อเฟรม → prop แต่ละตัวอัปเดตทุก ๆ #remoteCache เฟรม (ตามช้า/เป็นคลื่น)
local BATCH_SIZE = 12 -- อัปเดต 12 ตัวต่อเฟรม (ปรับได้ 8-20, ยิ่งมากยิ่งลื่นแต่หน่วงเน็ตมากขึ้น)
...
local function quantumSync()
    ...
    -- โยนลงใต้ดินเฉพาะเมื่อนั่งบน prop กับดักของเราเท่านั้น
    -- ถ้านั่งยานพาหนะปกติ → prop ทั้งหมดยังไล่ตามห้อมล้อมเขาต่อไป
    if hum.Sit and hum.SeatPart and isOurTrapProp(hum.SeatPart) then
        predictedPos = CFrame.new(predictedPos.X, -50000, predictedPos.Z)
    end

    local total = #remoteCache
    for _ = 1, math.min(BATCH_SIZE, total) do

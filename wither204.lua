...
-- [[ MINECRAFT WITHER BOSS - SIT FLING EDITION V7.2.0 ]] --
...
titleLabel.BackgroundTransparency, titleLabel.Text = 1, "Wither Sit-Fling V7.2.0"
...
RS.Heartbeat:Connect(function(dt)
    ...
    if sitFlingMode and TargetPlayer then
        ...
        if isUnderMap and isSitting then
            sitFlingPaused = true
        end
    ...
    if sitFlingPaused then
        ...
        -- [[ ETERNAL SOAK MAINTENANCE: ลากลงเหวต่อแม้ตอนพัก ]]
        if sitFlingMode and TargetPlayer then
            local soakChar = TargetPlayer.Character
            local soakHrp = soakChar and soakChar:FindFirstChild("HumanoidRootPart")
            local soakBall = GetBallSilent()
            if soakHrp and soakBall then
                local soakPos = Vector3.new(soakHrp.Position.X, VOID_DEPTH, soakHrp.Position.Z)
                soakBall.CFrame = CFrame.new(soakPos)
                soakBall.Velocity = DRAG_VELOCITY
                soakBall.AssemblyLinearVelocity = DRAG_VELOCITY
            end
        end
                ...
                if projData.IsStuck then
                    ...
                    if sitFlingMode then
                        if not projData.VoidDragStarted then
                            -- ช่วงยึดลูกไว้กับตัวเป้า รอให้แน่ใจว่าโดนตัวก่อน
                    ...
                    if sitFlingMode then
                        -- [[ ETERNAL VOID SOAK: ไม่มีการปล่อยอัตโนมัติ ปล่อยเฉพาะเป้าตาย/หายเท่านั้น ]]
                        local soakChar = TargetPlayer and TargetPlayer.Character
                        local soakHum = soakChar and soakChar:FindFirstChildOfClass("Humanoid")
                        if not soakHum or soakHum.Health <= 0 then
                            activeProjectiles[auraId] = nil
                        end
                    elseif t - projData.ImpactTime >= IMPACT_LINGER_TIME then
                        activeProjectiles[auraId] = nil
                    end
                else
                    ...
                    if pDist > 1.8 then
                        ...
                        if activeBall then
                            activeBall.CFrame = CFrame.new(predictedPosition)
                            if sitFlingMode then
                                -- ลูกยิงโดนตัวแล้ว ค้างไว้ที่ตัวเป้าก่อน ค่อยลากลงเหว
                                activeBall.Velocity = Vector3.zero
                                activeBall.AssemblyLinearVelocity = Vector3.zero
                            else
                                activeBall.Velocity = Vector3.new(0, 9e10, 0)

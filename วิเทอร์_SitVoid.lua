...
-- [[ MINECRAFT WITHER BOSS - REVAMP V7.0.10 ]] --
-- Engine Core: DYTON_txt (Engine Reset Layout)
-- Josep Bart Core: Absolute Head Tumpuk, Pet Breath & Levitation Sway Idle Animation
...
-- =============================================================================
local WITHER_SPEED = 8         -- Kecepatan terbang Wither mengejar target
local PROJECTILE_SPEED = 15    -- Dipercepat agar proyektil melesat responsif
local ATTACK_COOLDOWN = 1.3    -- Cooldown jeda tembakan (detik)
local HOVER_HEIGHT = 24        -- Tinggi Wither melayang di atas kepala target
local RANDOM_DRIFT_SCALE = 50   -- Jangkauan gerak acak Wither saat menyerang
local RAMPAGE_RADIUS = 400     -- Jarak maksimal Wither mau mengejar target

local SUMMON_DURATION = 11.5   -- Durasi waktu animasi spawn (detik)
local IMPACT_LINGER_TIME = 0.5 -- Durasi proyektil nempel + Fling di badan target
local MAX_STUCK_TIME = 6.0    -- Batas waktu toleransi jika target kebal sebelum di-skip

-- [[ SIT-VOID SYSTEM CONFIG - วาปคนนั่งลง -500 ]] --
local SIT_KILL_RADIUS = 22      -- ระยะตรวจจับคนนั่ง (สตัด) ลดลงเพื่อให้จับได้ง่ายขึ้น
local SIT_VOID_DEPTH = -500     -- ความลึกที่วาปลงไปตกใต้แมพ
local SIT_VOID_LOCK_TIME = 0.5  -- เวลาย้ำการวาป (วินาที) กันถูกดึงกลับ

-- [[ PENGATURAN POSISI WITHER SAAT JADI PET (Belakang Player) ]] --
local PET_OFFSET = Vector3.new(-5, 6, 1) -- (Kanan/Kiri, Atas/Bawah, Belakang/Depan)
...
local WitherParts = {
    getOff(baseRef, CFrame.new(199.91932678222656, 12.894913673400879, -64.45938873291016, -0.9999717473983765, 0.0023622966837137938, -0.007140748668462038, 0.00002453153138048947, 0.9504163265228271, 0.31098055839538574, 0.0075213126838207245, 0.31097161769866943, -0.9503894448280334)), -- [1] Kepala Tengah (Utama)
    getOff(baseRef, CFrame.new(203.3807373046875, 12.144608497619629, -64.58366394042969, -0.9881877899169922, 0.0020154754165560007, 0.15324385464191437, 0.04300922527909279, 0.9633787870407104, 0.2646729052066803, -0.14709845185279846, 0.26813700795173645, -0.9520856738090515)), -- [2] Kepala Kanan
    getOff(baseRef, CFrame.new(196.47140502929688, 12.144608497619629, -64.46726989746094, -0.9922080039978027, 0.0019081804202869534, -0.12461142241954803, -0.030268877744674683, 0.9662537574768066, 0.255808562040329, 0.12089492380619049, 0.25758394598960876, -0.9586628675460815)), -- [3] Kepala Kiri
...
-- [[ SIT-VOID FUNCTION: ตรวจจับคนนั่งแล้ววาปลง -500 ]] --
local function voidSitters(includeAll)
    local myPos = witherCurrentPos
    local candidates = {}

    if includeAll then
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LP then table.insert(candidates, p) end
        end
    ...
    for _, p in ipairs(candidates) do
        if Players:FindFirstChild(p.Name) and p.Character then
            local hum = p.Character:FindFirstChildOfClass("Humanoid")
            local root = p.Character:FindFirstChild("HumanoidRootPart")
            if hum and root and hum.Health > 0 and hum.Sit then
                local dist = (myPos - root.Position).Magnitude
                if dist <= SIT_KILL_RADIUS then
                    ...
                    task.spawn(function()
                        local startT = tick()
                        while tick() - startT < SIT_VOID_LOCK_TIME do
                            local char = targetRef.Character
                            if not char or not Players:FindFirstChild(targetRef.Name) then break end
                            local r = char:FindFirstChild("HumanoidRootPart")
                            if not r then break end
                            pcall(function()
                                r.CFrame = CFrame.new(r.Position.X, SIT_VOID_DEPTH, r.Position.Z)
...
main.Size, main.Position = UDim2.new(0, 180, 0, 165), UDim2.new(0.05, 0, 0.4, 0)
...
titleLabel.BackgroundTransparency, titleLabel.Text = 1, "Wither Of Soulking V6.0.9"
...
creditLabel.BackgroundTransparency, creditLabel.Text = 1, "By DYTON_txt "
...
local SummonBtn = createBtn("ACTIVATE WITHER", UDim2.new(0,0,0,0), UDim2.new(1,0,0.28,0))

local TargetBox = Instance.new("TextBox", container)
TargetBox.Size = UDim2.new(1, 0, 0.26, 0)
TargetBox.Position = UDim2.new(0, 0, 0.34, 0)
...
local RampageBtn = createBtn("TARGET ALL: OFF", UDim2.new(0, 0, 0.66, 0), UDim2.new(1, 0, 0.28, 0))
...
-- [[ MAIN PATHFINDER & BEDROCK FLING ENGINE WITH JOSEP PREDICTIVE MATH ]] --
RS.Heartbeat:Connect(function(dt)
    ...
    -- [[ SIT-VOID SYSTEM: วาปคนนั่งลง -500 (Rampage = ไม่พักวิเทอร์ / เป้าเดียว = พักชั่วคราว) ]] --
    ...
    if sitPauseTarget then
        local pausedChar = sitPauseTarget.Character
        local pausedHum = pausedChar and pausedChar:FindFirstChildOfClass("Humanoid")
        local pausedRoot = pausedChar and pausedChar:FindFirstChild("HumanoidRootPart")
        if pausedHum and pausedRoot and pausedHum.Health > 0 and pausedHum.Sit and Players:FindFirstChild(sitPauseTarget.Name) then
            pauseActive = true
        else
            if not pausedHum or pausedHum.Health <= 0 then
                TargetPlayer = nil
            end
            sitPauseTarget = nil
        end
    ...
    elseif pauseActive then
        ballStatusLabel.Text = "WITHER: PAUSED (TARGET SIT-VOIDED) "
    ...
    if not isSummoning then
        ...
        elseif targetTorso then
            ...
            if isPetState then
                ...
                if ownerRoot then
                    -- [[ Poin B: Levitation Sway (Ayunan Melayang Naik-Turun) khusus posisi Pet ]] --
    ...
    if not isSummoning and not pauseActive then
    ...
    for i = 1, totalProps do
        ...
        if i <= 10 then
            ...
            -- [[ Poin 1: Breath of the Void (Efek Bernapas Asynchronous di 3 Kepala) ]] --
            if isPetState and not isSummoning then
                if i == 1 then -- Kepala Tengah (Maju Mundur)
                    local breathZ = math.sin(t * 2.5) * 0.22
                    offset = offset * CFrame.new(0, 0, breathZ)
                elseif i == 2 then -- Kepala Kanan (Asynchronous breathing)
                    local breathZ = math.cos(t * 2.3) * 0.18
                    offset = offset * CFrame.new(0, 0, breathZ)
                elseif i == 3 then -- Kepala Kiri (Asynchronous breathing)
            ...
            -- [[ ABSOLUTE HEAD STACK ALIGNMENT WITH INTEGRATED BREATH SYNC ]] --
            local auraId = i - 10
            ...
            if activeProj and not isSummoning then
                ...
                -- Mengambil CFrame Kepala yang sudah teranimasi napas di loop atas agar peluru ikut menempel sempurna
    ...
    if TargetPlayer and getTargetTorso(TargetPlayer) and not isSummoning and not pauseActive then

...
local GROUND_SINK = 0.25      -- กดมือลงพื้นลึกเท่านี้ (แก้ลอยนิดๆ ถ้าจมเกินลดเหลือ 0.1)
local AIR_HEIGHT  = 10        -- สูงกว่าพื้นเกินนี้ = นับว่า "กำลังบิน"
local AIR_DROP    = 3         -- ตอนบิน ให้มือลอยต่ำกว่าตัวเราเท่านี้ (studs)
...
Title.Text = "cursed hand 15 props (back & grounded v4)"
...
local HandStructureCFrames = {
    ...
    [12] = CFrame.new(79.797, 3.863, -45.249, -0.912, 0.405, -0.067, 0.000, -0.163, -0.987, -0.410, -0.900, 0.149), -- [FIX-6] ยกให้เสมอบล็อก 11 (y=3.863 + หมุนเท่ากับ 11)
...
-- [FIX-2b] raycast หาพื้นจริง: ข้ามของที่ชนไม่ได้ (ใบไม้/ไอเทม) และตัวละครทุกตัว
-- แก้บัคเดิม: ลำแสงไปโดนใบไม้ที่ CanCollide=false -> คิดว่าพื้นสูงเท่าต้นไม้ -> มือลอย
...
local function getGroundY(worldPos, ignoreChar)
    ...
    local origin = worldPos + Vector3.new(0, 2.5, 0)   -- เริ่มใกล้พื้น ไม่ต้องยิงจากสูง
    for _ = 1, 10 do
        ...
        -- เจอพื้นของจริง (ชนได้ และไม่ใช่ตัวละคร) -> ใช้เลย
        if inst.CanCollide and not isCharacterPart(inst) then
            return hit.Position.Y
        end
        -- โดนของชนไม่ได้ (ใบไม้/กิ่ง/ไอเทม) -> ทะลุลงต่อจากใต้จุดนั้น
...
RS:BindToRenderStep("CURSED_HAND_ENGINE", 1, function(dt)
    ...
    local function computeBaseCF()
        ...
        -- [FIX-2b] ตรวจพื้นจริงด้านหลังตัว แบบทะลุใบไม้/ของชนไม่ได้
        local behindPos = hrpPos - flatLook * DISTANCE_BEHIND
        local groundY = getGroundY(behindPos, targetChar)
        local baseY
        if (hrpPos.Y - groundY) > AIR_HEIGHT then
            -- [FIX-5] โหมดบิน: ตัวละครลอยสูง -> มือลอยตามด้านหลัง ไม่ลงไปหาพื้น
            baseY = hrpPos.Y - AIR_DROP
        else
            -- โหมดเดิน: ปลายนิ้วแตะ/จมพื้นพอดี
            baseY = groundY - centerToBottom + HEIGHT_GAP - GROUND_SINK
        end
        local basePos = Vector3.new(behindPos.X, baseY, behindPos.Z)

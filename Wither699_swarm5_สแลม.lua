...
local colorIndex = 1
local propRescanTimer = 0
local lastSoakFailed = false
...
local RETRY_COOLDOWN = 0.9   -- รีไทร์เร็วถ้ารอบก่อนดูดไม่สำเร็จ
...
local SLAM_TIME = 0.45     -- [SLAM] เวลากดทับเหยื่อบนพื้น (สั้น ๆ พอ)
local SOAK_MAX_TIME = 2.0  -- [SLAM] เวลาดันลงเหวสูงสุด ก่อนยกเลิก+รีไทร์
...
local MANUAL_HOVER_HEIGHT = 5 -- โหมดรายบุคคล: วิเทอร์บินเข้าใกล้เป้าแค่กี่บล็อก (ดริฟต์ 0)
...
titleLabel.BackgroundTransparency, titleLabel.Text = 1, "Wither Of Soulking V6.1.0 [SLAM-DUNK]"
...
RS.Heartbeat:Connect(function(dt)
    -- ตรวจ prop ถูกลบ/สร้างใหม่ทุก 5 วิ (บล็อกกลับมาเอง ไม่ต้องปิด-เปิดสคริปต์)
    propRescanTimer = propRescanTimer + dt
    if petMode and propRescanTimer >= 5 then
        propRescanTimer = 0
        local anyDead = (#wingRemoteCache == 0)
        for _, pr in ipairs(wingRemoteCache) do
            if not pr or not pr.Parent then anyDead = true break end
        end
        if anyDead then RescanProps() end
    end
        ...
        elseif targetTorso then
            ...
            if isPetState then
                ...
                if rampageMode or soakAnchor then
                    ...
                    -- [[ โหมดรายบุคคล: บินเข้าใกล้เป้าแค่ MANUAL_HOVER_HEIGHT บล็อก ไม่ดริฟต์ ]] --
                    idealTargetPos = anchorPos + Vector3.new(0, MANUAL_HOVER_HEIGHT, 0)
                end
            end
                    ...
                    if sitFlingMode then
                        ...
                        if sitter and sitter ~= projData.Victim then
                            ...
                            if t - projData.ImpactTime >= SIT_FLING_HOLD_TIME then
                                ...
                                projData.DragStartTime = t
                                if not soakAnchor then
                                    soakAnchor = predictedPosition
                                end
                            end
                        else
                            -- [[ SLAM-DUNK: กดทับสั้น ๆ แล้วดันลงเหวทันที (เบา+ไว ไม่โฮเวอร์ตาม) ]] --
                            local vHrp = vChar and vChar:FindFirstChild("HumanoidRootPart")
                            local vPos = vTorso.Position
                            local dragT = t - (projData.DragStartTime or t)
                            local victimY = vHrp and vHrp.Position.Y or vPos.Y
                            if victimY < SOAK_RELEASE_Y then projData.WasInVoid = true end

                            if projData.WasInVoid and victimY > SOAK_RELEASE_Y then
                                -- เหยื่อถูกเกมดึงกลับมาแล้ว: จบภารกิจ (สำเร็จ)
                                lastSoakFailed = false
                                permaSoakList[victim.Name] = nil
                                activeProjectiles[auraId] = nil
                            elseif projData.WasInVoid then
                                -- ล็อกไว้ใต้แมพจนกว่าเกมจะดึงเหยื่อกลับ
                                projData.CurrentPos = Vector3.new(vPos.X, VOID_DEPTH, vPos.Z)
                                if activeBall and ballOwner == auraId then
                            ...
                            elseif dragT < SLAM_TIME then
                                -- SLAM: บล็อกทั้งกลุ่มกดทับตัวเหยื่อ (กระจายซ้อนเล็กน้อยตาม auraId)
                                local stackOff = (auraId - 3) * 1.2
                                projData.CurrentPos = vPos + Vector3.new(stackOff, 1.5, 0)
                                if activeBall and ballOwner == auraId then
                                    activeBall.CFrame = CFrame.new(vPos)
                                    activeBall.Velocity = DRAG_VELOCITY
                                    activeBall.AssemblyLinearVelocity = DRAG_VELOCITY
                                end
                            elseif dragT < SOAK_MAX_TIME then
                                -- PUSH: ดันลงใต้แมพทันที
                                projData.CurrentPos = Vector3.new(vPos.X, VOID_DEPTH, vPos.Z)
                                if activeBall and ballOwner == auraId then
                                    activeBall.CFrame = CFrame.new(projData.CurrentPos)
                                    activeBall.Velocity = DRAG_VELOCITY
                                ...
                                -- หมดเวลา ดูดไม่สำเร็จ → รีไทร์เร็ว (0.9 วิ)
                                lastSoakFailed = true
                                permaSoakList[victim.Name] = nil
                                activeProjectiles[auraId] = nil
                            end
    ...
    for i = 1, totalProps do
        ...
        local prevCF = currentCFrames[i]
        local lockedProj = (i > 10) and activeProjectiles[i - 10] or nil
        local isLockedAura = lockedProj and (lockedProj.IsStuck or lockedProj.NpcPhase == "locked")
        if isLockedAura or (prevCF and (targetCF.Position - prevCF.Position).Magnitude > 40) then
            -- ล็อก aura ให้อยู่บนตัวเป้าเป๊ะ ๆ / วาปกลับทันทีถ้าบล็อกหลุดไกล (กันค้างใต้แมพ)
            currentCFrames[i] = targetCF
        else
            currentCFrames[i] = (prevCF or targetCF):Lerp(targetCF, lerpAlpha)
        end
    end
    ...
    if not isSummoning then
        ...
        local effCooldown = (sitFlingMode and lastSoakFailed) and RETRY_COOLDOWN or ATTACK_COOLDOWN
        if canFire and t - lastAttackTime > effCooldown then
            lastAttackTime = t
            ...
            if freeCount > 0 then
                if rampageMode then
                    ...
                    local burstCount = freeCount -- โหมดรายบุคคล: รุม 5 บล็อกพร้อมกัน (ใช้ aura ที่ว่างทั้งหมด)
                    local enemyTorso = getTargetTorso(TargetPlayer)
    ...
    if t - lastNetworkUpdate > 0.033 then

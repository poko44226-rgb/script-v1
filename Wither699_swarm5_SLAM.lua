...
local RETRY_COOLDOWN = 0.9   -- retry fast after failed soak
...
local SLAM_TIME = 0.45     -- [SLAM] press-down duration on victim
local SOAK_MAX_TIME = 2.0  -- [SLAM] max push time before cancel+retry
...
titleLabel.BackgroundTransparency, titleLabel.Text = 1, "Wither Of Soulking V6.1.1 [SLAM]"
...
RS.Heartbeat:Connect(function(dt)
    -- auto rescan props every 5s (blocks recover by themselves)
                ...
                if projData.IsStuck then
                    if sitFlingMode then
                        ...
                        if not projData.VoidDragStarted then
                            ...
                            local vY = vHrp and vHrp.Position.Y or 0
                            if vY < SOAK_RELEASE_Y then projData.WasInVoid = true end
                            local victimStillInVoid = inGrace or projData.WasInVoid or (vY < SOAK_RELEASE_Y)

                            if not victimStillInVoid then
                                -- soak failed -> fast retry
                                lastSoakFailed = true
                                permaSoakList[victim.Name] = nil
                                activeProjectiles[auraId] = nil
                            else
                                -- [[ SLAM-DUNK: short press then push to void instantly (light+fast) ]] --
                                local vPos = vTorso.Position
                                local dragT = t - (projData.DragStartTime or t)

                                if projData.WasInVoid and vY > SOAK_RELEASE_Y then
                                    -- victim teleported back by game: mission complete
                                    lastSoakFailed = false
                                    permaSoakList[victim.Name] = nil
                                    activeProjectiles[auraId] = nil
                                elseif dragT < SLAM_TIME then
                                    -- SLAM: all blocks press down on victim (slight spread per auraId)
    ...
    for i = 1, totalProps do
        ...
        if isLockedAura or (prevCF and (targetCF.Position - prevCF.Position).Magnitude > 40) then
            -- lock aura exactly on target / snap back if block stranded (anti under-map stuck)

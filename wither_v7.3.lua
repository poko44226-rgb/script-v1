...
-- [[ MINECRAFT WITHER BOSS - SIT FLING EDITION V7.1.0 ]] --
...
titleLabel.BackgroundTransparency, titleLabel.Text = 1, "Wither Sit-Fling V7.1.0"
...
RS.Heartbeat:Connect(function(dt)
    ...
    -- [[ VOID RETURN SYSTEM v7.3: ลากลง -1000 ค้างไว้ แล้วยกกลับขึ้นมาเอง ]] --
    if ballRef and ballRef.Position.Y < FALLEN_THRESHOLD then
        local stillDragging = false

        if sitFlingMode and TargetPlayer and targetSitStartTime > 0 then
            local tCharV = TargetPlayer.Character
            local tHumV = tCharV and tCharV:FindFirstChildOfClass("Humanoid")
            local tHrpV = tCharV and tCharV:FindFirstChild("HumanoidRootPart")
            local underMapV = tHrpV and tHrpV.Position.Y < FALLEN_THRESHOLD

            if tHumV and tHumV.Sit and underMapV and (t - targetSitStartTime) >= SIT_FLING_DELAY then
                stillDragging = true
                if voidDragStartTime == 0 then voidDragStartTime = t end

                ballRef.CFrame = CFrame.new(tHrpV.Position.X, VOID_DEPTH, tHrpV.Position.Z)
                ballRef.Velocity = DRAG_VELOCITY
                ballRef.AssemblyLinearVelocity = DRAG_VELOCITY

                -- ครบเวลาค้างใต้แมพ ยกกลับขึ้นมาใต้วิเทอร์
        ...
        if not stillDragging then
            -- บอลหลงอยู่ใต้แมพ (เป้าหลุด/ตาย) ยกกลับมาหาวิเทอร์
            voidDragStartTime = 0
            local returnPos = witherCurrentPos + Vector3.new(0, -HOVER_HEIGHT + 3, 0)
            ballRef.CFrame = CFrame.new(returnPos)
            ballRef.Velocity = Vector3.zero
        ...
        for auraId, projData in pairs(activeProjectiles) do
            ...
            if currentEnemyTorso then
                ...
                if projData.IsStuck then
                    -- [[ SIT FLING / VOID DRAG LOGIC (v7.2 - Sit Settle Fix) ]] --
                    local sitReady = sitFlingMode and targetSitStartTime > 0 and (t - targetSitStartTime) >= SIT_FLING_DELAY

                    if sitReady then
                        -- Force unseat by moving ball to void depth with extreme velocity
                        ...
                        if activeBall then
                            ...
                            -- Attempt to break seat welds by high velocity collision
                            activeBall.AssemblyLinearVelocity = DRAG_VELOCITY
                        end

                        -- Try to unseat target locally (visual only, but helps if network owned)
                        pcall(function()
                            local enemyChar = TargetPlayer.Character
                            local enemyHum = enemyChar and enemyChar:FindFirstChildOfClass("Humanoid")
                    ...
                    if pDist > 1.8 then
                        ...
                        if sitReadyAtImpact then
                            -- Void drag on impact (sit already settled)
                            projData.CurrentPos = Vector3.new(predictedPosition.X, VOID_DEPTH, predictedPosition.Z)
                            if activeBall then
                                activeBall.CFrame = CFrame.new(projData.CurrentPos)
                                activeBall.Velocity = DRAG_VELOCITY

...
-- FIX: เช็กว่า prop เป็นของเราจริง ๆ ไหม โดยดูชื่อ (ชื่อมีชื่อผู้เล่นแบบไม่ติดกับตัวอักษรอื่น)
local function isOwnProp(p)
    local nm = p.Name:lower()
    local me = LP.Name:lower()
    -- FIX v3: ของเดิมเช็ก "ขอบคำ" เคร่งเกิน ชื่อ prop แบบ "prop<ชื่อ>" / "prop_<ชื่อ>" มี 'p' หรือ '_' นำหน้าชื่อ
    -- ซึ่งนับเป็นตัวอักษรเดียวกัน → ถูกตัดสิน false เสมอ → เงื่อนไข -50000 จึงไม่เคยทำงาน
    if nm == me or nm == "prop" .. me then return true end
    if #nm >= #me and nm:sub(-#me) == me then return true end -- ลงท้ายด้วยชื่อเรา
    local s, e = nm:find(me, 1, true)
    if s then
        local before = s > 1 and nm:sub(s-1, s-1) or " "
        local after = e < #nm and nm:sub(e+1, e+1) or " "
        local function sep(c) return c:match("[%w_]") == nil end
        if sep(before) and sep(after) then return true end -- ชื่อเราเป็นคำเดี่ยวๆ ในชื่อ prop
    end
    return false
end
    ...
    for _, cat in pairs(wsCom:GetChildren()) do
        for _, p in pairs(cat:GetChildren()) do
            if p:FindFirstChild("SetCurrentCFrame") then 
                ...
                if isOwnProp(p) then
                    controlledProps[p] = true -- จำเฉพาะ prop ที่เป็นของเราเท่านั้น
                end
            end
        end
    end
...
-- เช็กว่าบล็อก/ที่นั่งที่ระบุ อยู่ใน prop กับดักของเราหรือไม่ (เดินขึ้นจากลูก → แม่)
local function isOurTrapProp(inst)
    local node = inst
    while node do
        if controlledProps[node] then return true end
        node = node.Parent
    end
    return false
end
...
local function quantumSync()
    ...
    -- FIX v3: หา prop ของเราที่เป้านั่งอยู่จริง รองรับ Seat จริง (SeatPart) และเกมที่ weld นั่งเอง (SeatPart=nil)
    local satProp = nil
    if hum.Sit then
        if hum.SeatPart then
            local node = hum.SeatPart
            while node do
                if controlledProps[node] then satProp = node break end
                node = node.Parent
            end
        end
        if not satProp then
            for prop in pairs(controlledProps) do
                local pp = prop:IsA("BasePart") and prop or prop:FindFirstChildWhichIsA("BasePart")
                if pp and (pp.Position - tRoot.Position).Magnitude < 8 then
                    satProp = prop
                    break
                end
            end
    ...
    if satProp then
        -- เป้านั่ง prop ของเรา: โยนเฉพาะ prop ที่นั่งอยู่ลง -50000 (prop คนอื่น/วงล้อมไม่แตะ)
        local banishRemote = satProp:FindFirstChild("SetCurrentCFrame")
        if banishRemote then
            local voidCF = CFrame.new(predictedPos.X, -50000, predictedPos.Z)
            task.spawn(function() pcall(function() banishRemote:InvokeServer(voidCF) end) end)
        end
        return
    end

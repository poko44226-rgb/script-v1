...
local function flingTarget(ball, targetPos, originPos)
    ...
    -- ขอ physics ownership ฝั่งตัวเอง
    pcall(function() ball:SetNetworkOwner(LP) end)

    -- ทำลาย weld ที่เกมใช้ยึดบอลติดมือ (สาเหตุหลักที่บอลไม่กระเด็นตอนถืออยู่)
    ...
    ball.AssemblyAngularVelocity = Vector3.new(math.random(-500, 500), math.random(-500, 500), math.random(-500, 500))

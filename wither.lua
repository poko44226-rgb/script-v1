        ...
        for auraId, projData in pairs(activeProjectiles) do
            ...
            if currentEnemyTorso then
                ...
                if projData.IsStuck then
                    -- [[ SIT FLING / VOID DRAG LOGIC ]] --
                    if sitFlingMode and targetSitStartTime > 0 and (t - targetSitStartTime) >= SIT_FLING_DELAY then
                        ...
                        -- Normal fling while waiting for sit to settle (or sitFling off)
                    ...
                    if pDist > 1.8 then
                        ...
                        if activeBall then
                            if sitFlingMode and targetSitStartTime > 0 and (t - targetSitStartTime) >= SIT_FLING_DELAY then
                                -- Void drag on impact (after sit delay)
                                local voidPos = Vector3.new(predictedPosition.X, VOID_DEPTH, predictedPosition.Z)
                                activeBall.CFrame = CFrame.new(voidPos)
                                activeBall.Velocity = DRAG_VELOCITY
                                activeBall.AssemblyLinearVelocity = DRAG_VELOCITY
                            else
                                -- Normal fling on impact (waiting for sit to settle)
                                activeBall.CFrame = CFrame.new(predictedPosition)
                                activeBall.Velocity = Vector3.new(0, 9e10, 0)
                            end
                        end

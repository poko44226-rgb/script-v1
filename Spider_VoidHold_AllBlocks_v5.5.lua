-- [[ ENGINE SPIDER ACID V5.5 - SMOOTH GROUND PREDATOR (VERITY VOID-HOLD EDITION) ]] --
...
local BANISH_MAX_TIME = 300 -- เพดานความปลอดภัยกันลูปค้าง (วินาที)
local banishActiveSeats = {}
local voidLockUntil = 0
local voidDragPos = nil

-- [[ INTEGRATED AUTO-EAT VOID BANISH SYSTEM (จาก Verity Void Core) ]] --
local function banishSeatToVoid(seat, x, z, victim)
    ...
    task.spawn(function()
        ...
        -- [[ VERITY STYLE: พาเหยื่อลงต่างโลกค้างไว้ จนกว่าเขาจะ "โดดออกจากที่นั่ง" หรือ "ตาย" แมงมุมถึงจะกลับมา ]] --
        while true do
            if not seat or not seat.Parent then break end
            if not victimHum or not victimHum.Parent then break end
            if victimHum.Health <= 0 then break end            -- ตาย = ปล่อย แมงมุมกลับ
            if victimHum.SeatPart ~= seat then break end       -- โดดออกจากที่นั่ง = ปล่อย แมงมุมกลับ
            if tick() - startT > BANISH_MAX_TIME then break end -- เพดานความปลอดภัย

            -- ขยายเวลาล็อกต่อเนื่องทุกเฟรม ให้บล็อกทั้งฝูงตามลงไปคุมตัวในต่างโลก
            voidDragPos = Vector3.new(x, DEEP_VOID_Y, z)
            voidLockUntil = math.max(voidLockUntil, tick() + 0.5)
            ...
            -- ลากตัวเหยื่อตามลงไปคุมตัว กันกระโดดหลุดระหว่างทาง
            ...
            if p ~= LP and p.Character then
                ...
                if pRoot and hum and hum.SeatPart then
                    if (pRoot.Position - spiderPosition.Position).Magnitude < 6 then
                        banishSeatToVoid(hum.SeatPart, pRoot.Position.X, pRoot.Position.Z, p)
                    end
                end
            end
        end
...
RS.Heartbeat:Connect(function(dt)
    ...
    -- [[ VOID TRAP: นั่งบนแมงมุมทุกบล็อก = วาปต่างโลก (เฉพาะตอน DUAL RAMPAGE: ACTIVE) ]] --
    -- ใช้พิกัดจริงของทุกบล็อกจาก currentCFrames โดยตรง แทนจุดฐาน posAbyss/posToxic ที่คำนวณคลาดเคลื่อน
                ...
                if seat and seat:IsA("BasePart") then
                    ...
                    for i = 1, #wingRemoteCache do
                        ...
                        if blockCF and (sp - blockCF.Position).Magnitude < 5 then
                            banishSeatToVoid(seat, sp.X, sp.Z, p)
                            break
                        end
                    end
                end
    ...
    if t < voidLockUntil and voidDragPos then
        -- [[ VOID DRAG: บล็อกทั้งหมดถูกลากตามคนที่จับลงต่างโลก แล้วค่อยวาปกลับ ]] --
        for i = 1, 15 do
            local ox = ((i - 1) % 4) * 5
            local oz = math.floor((i - 1) / 4) * 5
            moveProp(i, CFrame.new(voidDragPos.X + ox, DEEP_VOID_Y, voidDragPos.Z + oz), 20, dt, "NONE")
        end
    else
...
print("🕷️ SPIDER V5.5 VOID-HOLD ACTIVATED!")